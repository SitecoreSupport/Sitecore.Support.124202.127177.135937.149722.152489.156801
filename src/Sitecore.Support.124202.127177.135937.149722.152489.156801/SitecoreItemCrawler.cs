...
namespace Sitecore.Support.ContentSearch
{
  ...
  public class SitecoreItemCrawler : HierarchicalDataCrawler<SitecoreIndexableItem>, IContextIndexRankable
  {
    ...
    /// <summary>
    /// Path: This method retrieves the versions for a specific item.
    /// If there are no versions available but the item is fallbacked, then retrieve the fallbacked item as it's only version.
    /// </summary>
    /// <param name="item">The item.</param>
    /// <returns>List of items to index.</returns>
    private Item[] GetItemVersions(Item item)
    {
      var versions = item.Versions.GetVersions(false);
      if (versions.Length == 0 && item.IsFallback)
      {
        versions = new[] { item };
      }
      ...
      using (new LanguageFallbackItemSwitcher(context.Index.EnableItemLanguageFallback))
      {
        ...
        if (!this.IsExcludedFromIndex(indexable))
        {
          ...
          foreach (var language in indexable.Item.Languages)
          {
            ...
            using (new WriteCachesDisabler())
            {
              versions = this.GetItemVersions(latestVersion); // Patch: the call to this method has been changed
            }
    ...
    // Patch 156801: LanguageFallbackItemSwitcher context has been added
    ...
    {
      Func<Language, IEnumerable<Item>> selector = null;
      SitecoreIndexableItem indexableItem = this.GetIndexable(indexableUniqueId) as SitecoreIndexableItem;
      if (indexableItem != null)
      {
        ...
        if (indexableItem.Item.Fields.Any<Field>(e => e.SharedLanguageFallbackEnabled))
        {
          if (selector == null)
          {
            selector = delegate (Language language)
            {
          ...
          foreach (Item item in LanguageFallbackManager.GetDependentLanguages(indexableItem.Item.Language, indexableItem.Item.Database, indexableItem.Item.ID).SelectMany<Language, Item>(selector))
          {
      ...
      var indexableItem = LanguageFallbackManager.GetDependentLanguages(item.Language, item.Database, item.ID)
        .SelectMany(
          language =>
          {
            ...
            var items = item1 != null ? (IEnumerable<Item>)this.GetItemVersions(item1) : new Item[0]; // Patch: the call to the GetItemVersions method has been changed
            return items;
          })
    ...
    // Patch 127177: LanguageFallbackItemSwitcher context has been added
